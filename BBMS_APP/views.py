...
def userLogin(request):
    if request.method == 'POST':
        ...
        try:
            ...
            return redirect('login')
    return render(request, 'BBMS_APP/main/login.html')
# Custom User login_code end here
    ...
    try:
        ...
        if customer_id:
            ...
            if request.method == "POST":
                ...
                client = razorpay.Client(auth=('rzp_live_0jwn9ZE5tjxHgg', 'GNEatZeKC6hvkPmojM2c2sOw'))
...
# Admin page Customer order history
def adminBloodOrderHistory(request):
    order_history = CustomerOrderDetails.objects.all()
    records = {
        'history': order_history,
    }
    return render(request, 'BBMS_APP/admin/user/admin_user_blood_order_request_history_page.html', records)
# Admin page Customer order history end

# ************************************************************** Profile & Logout code *********************************************************************
# Profile and Logout code
@login_required
def profile(request):
    user_id = request.session.get('user_id', None)  # Retrieve user ID from session
    user_data = {}
    if request.method == 'POST' and 'logout' in request.POST:
        ...
        try:
            if user_id:
                ...
                login_sessions = CustomerLoginSession.objects.filter(user=user)
                ...
                raise CustomerDetails.DoesNotExist
        except CustomerDetails.DoesNotExist:
            messages.error(request, "User not found", extra_tags="user_not_found")
        return redirect('index')
...
@login_required
def pricing(request):
    return render(request, 'BBMS_APP/for_all_pricing.html')
...
# Donor details page
@login_required
def adminDonorDetails(request):
    return render(request, 'BBMS_APP/admin/donor/admin_donor_details_page.html')

# Donor details record page
@login_required
def adminDonorDetailsDetails(request):
    if request.method == 'POST':
        print(request.POST)
        if 'delete' in request.POST:
            ...
            try:
                ...
                return redirect('adminDonorDetailsDetails')
    ...
    return render(request, 'BBMS_APP/admin/donor/admin_donor_details_details_page.html', context)
# Donor details record page end here

# Donor record modification page
@login_required
def adminDonorDetailsModification(request):
    donor_records = DonorDetails.objects.all()
    district = MainBloodBankSystem.objects.all()
    blood_group = BloodGroupRecord.objects.all()
    records = {
...
def adminUserDetails(request):
    return render(request, 'BBMS_APP/admin/user/admin_user_details_page.html')


@login_required
def adminUserDetailsDetails(request):
    ...
    if request.method == 'POST':
        if 'delete' in request.POST:
            ...
            return redirect('adminUserDetailsDetails')
            pass
    return render(request, 'BBMS_APP/admin/user/admin_user_details_details_page.html', records)


# ***********************************************************************************************************************************
@login_required
def bloodStockDetails(request):
...
# ***********************************************************************************************************************************
@login_required
def adminDoctorDetails(request):
    return render(request, 'BBMS_APP/admin/doctor/admin_doctor_details_page.html')
...
def doctorDetailsDetailsPage(request):
    doctor_records = DoctorDetails.objects.all()
    records = {
        'doctors': doctor_records,
    }
